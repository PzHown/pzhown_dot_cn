...
description: 为 pzhown_dot_cn 统一所有可交互元素的“可操作感”和状态表达。用于正文链接、导航链接、Text Action、Button、Icon Button、Clickable Surface/Card、SegmentedControl、TabBar、Toggle、Menu Item，以及 hover、pressed、focus-visible、selected、disabled、pending 等交互状态。目标是在不把所有元素都做成同一种按钮的前提下，让用户无需试点就能判断哪里可操作，并区分导航、动作与状态变化；同时兼顾 Mouse、Touch、Keyboard 和原生 Web 语义。
---
...
- **原生语义优先**：能用 `<a>`、`<button>`、`<input>`、`<select>`、`<progress>` 等表达时先用原生语义，不使用 `div onClick` 或无必要的 ARIA role 模拟控件。
- 共享实现优先使用 `@pzhown/ui` 的 PzHown-owned iOS 27 primitives；不要恢复旧 shadcn / aria-nova / Base UI 组件链。

## 工作流程

1. 判断交互意图：导航、动作、状态切换、选择、菜单命令，还是进入一个对象。
2. 选择正确语义元素：`<a>` / Link、`<button>`、Switch、原生表单控件、Tab/Segmented、MenuItem 等。
...
8. 如果原生语义无法可靠承担复杂 Overlay、Composite Widget 或焦点管理，才按需引入成熟行为 primitive；行为层不能成为第二套视觉系统。
...
- 同一任务区只保留一个最强主操作。
- Pressed 必须即时可感知。
- iOS 27 的视觉尺寸服从 `DESIGN.md`；触控命中区可以独立扩展。
...
- Surface 内存在 Menu、Bookmark、Checkbox 等次级操作时，不把整块包进一个 `<a>` 导致嵌套交互冲突。
...
- **Invalid**：错误状态优先于普通 Focus 色，并配合可理解的文本/图标，不只变红。
...
- 正文 Link 的密度、行长和阅读影响：`perceptual-reading`。
- Card/内容块应不应该存在：`content-presentation`。
- 控件内部 inset、icon-label gap、相邻目标间距和静态重心：`spatial-composition`。
- 不同空间的重排与输入能力：`adaptive-layout`。
- iOS 27 颜色、圆角、Surface、材质：`apple-design` + `DESIGN.md`。
- Presence、共享元素、页面过渡和复杂 gesture：`interaction-motion`。

## 参考资料

- 统一交互语法：`references/interaction-grammar.md`。
- Link、Text Action 与 Button 的边界：`references/links-actions.md`。
- hover / focus / pressed / selected 等状态：`references/states-feedback.md`。
- 触控命中区、输入能力与可访问性：`references/hit-targets.md`。
...
- 是否优先复用了新 `@pzhown/ui`，而不是恢复旧组件实现？
- 静止状态是否已有适量可操作线索？
- 正文 Link 是否脱离颜色仍能辨认？
- Hover 是否只是增强，而不是唯一发现方式？
- Focus-visible 是否清楚且不被裁切？
- Pressed 是否即时、短促、不会阻塞？
- Selected / Disabled / Invalid / Pending 是否与默认状态清楚区分？
- 触控命中区是否足够，且没有误触高风险？
- 整块可点击 Surface 是否与内部次级操作冲突？
- 是否无必要地引入第三方行为/样式体系，而不是使用原生语义？
